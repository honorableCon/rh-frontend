...
import { getAllContrats, addPersonnel } from '../../service/graphql';
import { personnelSchema } from '../../service/validator';
...
const add = ({ data }) => {
  ...
  const [{ errors, success, personnelId }, setQueryResult] = useState({
    errors: null,
    success: false,
  });
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    errors = {};
    const { error, value } = personnelSchema.validate(inputs, {
      abortEarly: false,
    });
    {
    ...
    if (error) {
      ...
      addPersonnel(value).then((res) => {
        setQueryResult({
          errors: null,
          success: true,
          personnelId: res.data.personnel.id,
        });
      });
    }
  };
  ...
  return (
    <div className="my-6">
      ...
      <form onSubmit={handleSubmit} ref={formRef} method="POST">
        ...
        {page == 3 && <Recap inputs={inputs} usePage={[page, setPage]} />}
        {success && (
          <ToastSuccess
